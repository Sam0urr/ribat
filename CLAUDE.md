...
1. **No build step.** `web/index.html` is one hand-readable file. No React, no
   bundler, no npm, no new runtime dependencies. Vendored assets only; the
   published page makes zero third-party requests at runtime.
...
3. **Weights are lagged.** Each month uses the latest benchmark year strictly
   ...
   weights. Non-reporters carry forward their last benchmark and must be
   listed in `stale_weights` and flagged in the UI (currently: RUS, 2021).
...
7. **Known-gap honesty.** Taiwan: risk source only (Comtrade non-reporter,
   ...
   Coverage (`covered_share`) is reported next to every value it qualifies.
...
banned-token residue, and licence presence.
