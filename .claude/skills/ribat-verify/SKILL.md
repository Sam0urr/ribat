...
6. **Licence and attribution** — LICENSE and SOURCES.md exist and still carry
   ...
   publish.
7. **Export contract** — `index.html` still has the CSV/JSON export machinery,
...
8. **Reverse map (source attribution)** — the attribution function, the
...
9. **Provenance** — README's generated block carries the series digest of
...
10. **Pipeline guards** — 03 has `guard_no_regression`, `--allow-regression`
...
11. **Export contract (map)** — `preserveDrawingBuffer` is set only inside
...
12. **Map coverage** — every economy in every payload maps to a paintable
    ...
    being coloured.
13. **Methods-review contract** — `intensity.json` carries
