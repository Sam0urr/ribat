...
or its typeset copy at [sam0urr.github.io/ribat/method.html](https://sam0urr.github.io/ribat/method.html)
(generated by `pipeline/05_render_method.py`; the verifier fails if the two diverge).
See [`SOURCES.md`](SOURCES.md) for the data manifest and licences.

Data requests, methodology questions, bug reports and anything else go through
the [issue forms](https://github.com/Sam0urr/ribat/issues/new/choose). They are
public and need no server, which is what keeps the site free of third-party
requests; no personal mailbox is published.
...
| | |
|---|---|
| GPR workbook | `data_gpr_export.xls`, 2,714,112 bytes |
| Workbook SHA-256 | `6ff388db4aa34eb66433d4c13c9f82d88316714889a1fc9e6a8c0ce1158d5eae` |
| Fetched | 2026-09-03 |
| Data through | 2026-08 |
| Series SHA-256 | `5ddbcd6e070e94647f2ba99086d706c26c24832ccf3434437c3e0b921bc31f70` |
| Payload built | 2026-09-03 |
...
Everything the two pages load is vendored — country polygons and every payload
under `web/data/`; the two libraries (MapLibre GL JS 5.24.0, topojson-client
3.1.0) and the story page's typefaces (Fraunces, IBM Plex Sans, IBM Plex Mono)
under `web/vendor/`, with a hash manifest (`web/vendor/MANIFEST.json`) — so the
published site makes zero third-party requests at runtime: no CDN, no font
service, no tile server, API or tracker. Upgrades happen by replacing the
vendored file and its manifest entry, never by a CDN tag (see `SOURCES.md`,
Rendering).
...
└── web/
    ...
    ├── vendor/                  MapLibre, topojson-client, fonts + MANIFEST.json (hashes)
